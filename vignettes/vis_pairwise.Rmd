...
However, this is mostly because two of the species (Gentoo and Chinstrap) are located on one island only. For these species, the score values for island  and the the other variables is NA, shown in grey.
...
Similarly `age:income` has a high ace score, and a plot of these two variables shows income goes up with aage until about age 40 and then drops off.

<!-- Note that for categorical pairs of variables, the highest value is given by uncertainty in all cases. -->

<!-- ```{r} -->
<!-- scores |> mutate(valmax = max(abs(value)), .by=c(x,y)) |> -->
<!--   filter(pair_type=="ff") |> -->
<!--   group_by(x,y)|>  -->
<!--   slice_max(value) |> arrange(desc(value)) -->
<!-- ``` -->


Next, we calculate scores by race and filter those x,y pairs with high differences:
...
mutate(group_scores, valrange = rng(value),valmax = max(abs(value)), .by=c(x,y))|>
  ...
  plot(type="linear", geom="point", interactive=TRUE, pair_order = "seriate_max_diff")
```
...
ggplot(data=acs12, aes(x=employment, y=hrs_work))+
  geom_boxplot()+
  facet_grid(rows=vars(race)) # NA col is because for employed=NA, there  hrs_work also NA
```
